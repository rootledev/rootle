...
with sha-keyed disk cache (`~/.cache/rootle/trees/<sha>.json`), ref→sha
...
**Note: rootle forces colors on** (`Colored::set_ansi_color_disabled(false)`)
...
blob to `~/.cache/rootle/edit/<owner>__<repo>/<path>` (traversal-safe),
...
`--theme <NAME>` (external palette files: `~/.config/rootle/themes/<name>.toml`
...
per `.agents/skills/rootle-tui-debug` (hub PTY, send keys ONE at a time —
back-to-back ESC bytes can merge into `Alt+<key>` in crossterm's parser);
scaffold new components per `.agents/skills/rootle-component`.
...
content-addressable disk cache under `~/.cache/rootle`.
...
launch
  ...
  ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄ modeline + key hints ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
        ...
        <Enter> on file → open in editor (read-only), exit → back to rootle
...
rootle is strictly modal. Global mode stack; the modeline always shows the
...
## 3. Config (`~/.config/rootle/config.toml`)

Loaded via the `dirs` crate (respects XDG; `$ROOTLE_CONFIG` overrides).
...
- **Palette is external to the app.** Themes are TOML palette files.
  Resolution order for `[theme].name`: `~/.config/rootle/themes/<name>.toml`
  → `<config dir>/themes/<name>.toml` → embedded fallback.
- Default: **Catppuccin Mocha**, embedded as the fallback so the binary
  runs standalone; on first run rootle also writes
  `~/.config/rootle/themes/catppuccin-mocha.toml` so users can fork it.
...
src/
  ...
  state.rs          // persisted app state (~/.local/state/rootle)
  editor.rs          // suspend/resume terminal, spawn editor
  config.rs          // ~/.config/rootle/config.toml (serde + defaults)
```
...
- Auth token resolution order: `ROOTLE_TOKEN` → `GITHUB_TOKEN` → `gh auth token`
...
`~/.local/state/rootle/state.json` (XDG state dir; small, human-readable,
...
- On launch with a `last_repo`, rootle offers "resume" via the search
...
1. Materialize the blob from cache (or fetch) into
   `~/.cache/rootle/edit/<repo-slug>/<path>` so the user sees a real filename.
2. Suspend: leave alternate screen, disable raw mode.
3. Spawn editor **read-only** (decided: rootle is a browser/viewer — `vim -R`,
...
- Release flow: `git tag vX.Y.Z` → CI (GitHub Actions) runs
  `docker compose run --rm release` → `gh release create vX.Y.Z dist/rootle-*`
