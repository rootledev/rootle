...
impl ProviderReference {
    pub fn parse(input: &str) -> Result<ProviderReference> {
...
fn parse_tarball_url(url: &str, tag: Option<String>) -> Result<ProviderReference> {
...
pub(super) fn binary_name_of(r: &ProviderReference) -> String {
...
mod tests {
    ...
    fn ref_grammar() {
        ...
        assert_eq!(
            ProviderReference::parse("gitlab").unwrap(),
            ProviderReference {
        ...
        assert_eq!(
            ProviderReference::parse("rootledev/rootle-gitlab").unwrap(),
            ProviderReference {
                repo: "rootledev/rootle-gitlab".into(),
                name: "gitlab".into(),
                tag: None,
        ...
        // Full URL.
        assert_eq!(
            ProviderReference::parse("https://github.com/rootledev/rootle-gitlab").unwrap(),
            ProviderReference {
                repo: "rootledev/rootle-gitlab".into(),
                name: "gitlab".into(),
                tag: None,
        ...
        // Tag pin.
        assert_eq!(
            ProviderReference::parse("rootledev/rootle-gitlab@v0.1.0")
                .unwrap()
                .tag,
            Some("v0.1.0".into())
        );
        // Bare name with tag.
        assert_eq!(
            ProviderReference::parse("gitlab@v0.2.0").unwrap().tag,
            Some("v0.2.0".into())
        );
        // Malformed.
        assert!(ProviderReference::parse("").is_err());
        assert!(ProviderReference::parse("justtext/").is_err());
        // Unprefixed repo: stem is the name.
        assert_eq!(
            ProviderReference::parse("someone/myprovider").unwrap().name,
            "myprovider"
        );
    }
    ...
    fn plain_http_tarball_urls() {
        ...
        let r = ProviderReference::parse(&bare).unwrap();
        assert_eq!((r.name.as_str(), r.tag), ("gitlab", None));

        // An explicit @tag wins over the filename.
        let pinned = ProviderReference::parse(&format!("{url}@v9.9.9")).unwrap();
        assert_eq!(pinned.tag, Some("v9.9.9".into()));

        // Wrong platform / not a tarball: a crisp user error.
        assert!(
            ProviderReference::parse("https://artifacts.corp.example/rootle-gitlab.tar.gz")
                .is_err()
        );
        assert!(ProviderReference::parse("https://artifacts.corp.example/rootle-gitlab").is_err());
    }
}
