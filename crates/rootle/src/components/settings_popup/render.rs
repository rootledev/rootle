...
impl SettingsPopup {
    pub fn render(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let hints = if self.editing.is_some() || self.filter.active() {
            crate::keymap::hints(crate::mode::Mode::Search)
        } else {
            crate::keymap::list_hints(crate::keymap::ListContext::Settings, false)
        };
        let hint = crate::keymap::hint_row(hints);
        let mut block = Block::default()
            ...
            .title(Span::styled(
                if self.filter.active() || !self.filter.is_empty() {
                    format!(" settings /{} ", self.filter.text())
                } else {
                    " settings ".to_string()
                },
    ...
    fn render_section(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        self.viewport.render(
            ...
            inner,
            lines,
            cursor_line.map(RowSpan::single),
            theme,
        );
        let scroll = self.viewport.offset().get();
    ...
    fn section_lines(&self, width: usize, theme: &Theme) -> (Vec<Line<'static>>, Option<usize>) {
        ...
        for (visible_index, source_index) in self.visible_rows().into_iter().enumerate() {
            let row = &self.sections[self.section].rows[source_index];
            ...
            let selected = visible_index == self.selection.selected().get();
            let bg = if selected { sem.selection_bg } else { sem.base };
            let gutter = selection_gutter(selected, sem);
            let label = |label: &str| {
                Span::styled(
                    format!("{label:<9}  "),
                    Style::default().fg(sem.subtext0).bg(bg),
            ...
            match row {
                ...
                Row::Radio { group, option, .. } => {
                    ...
                    let option_style = if selected {
                        selection_style(true, sem).add_modifier(modifier)
                    } else {
