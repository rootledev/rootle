...
impl VimInput {
    ...
    fn insert_key(&mut self, key: KeyEvent) -> Outcome {
        match crate::keymap::input_control(key, false, self.modal) {
            Some(crate::keymap::InputControl::Submit) => return Outcome::Submitted,
            Some(crate::keymap::InputControl::Escape) => {
                if !self.modal {
    ...
    fn normal_key(&mut self, key: KeyEvent) -> Outcome {
        ...
        match crate::keymap::input_control(key, true, self.modal) {
            Some(crate::keymap::InputControl::Insert) => {
                self.submode = SubMode::Insert;
                return Outcome::Noop;
            }
            Some(crate::keymap::InputControl::Escape) => return Outcome::Cancelled,
            _ => {}
        }
        match key.code {
