...
use crate::components::list_view::{RowSpan, selection_gutter, selection_style};
...
impl CloneWizard {
    pub fn render(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let title = if self.filter.active() || !self.filter.is_empty() {
            format!("{} /{} ", title.trim_end(), self.filter.text())
        } else {
            title.to_string()
        };
        ...
        let (lines, selected) = match self.screen {
            Screen::Repos => (
                self.repos_lines(theme),
                (!self.visible_repos().is_empty())
                    .then(|| RowSpan::single(self.selection.selected().get())),
            ),
            Screen::Destination => (
                self.destination_lines(theme),
                (!self.visible_dest().is_empty())
                    .then(|| RowSpan::single(1 + self.destination_selection.selected().get())),
            ),
            Screen::Summary => (self.summary_lines(theme), None),
        };
        self.viewport
            .render(frame, popup, rows[1], lines, selected, theme);
    ...
    fn repos_lines(&self, theme: &Theme) -> Vec<Line<'static>> {
        ...
        let lines: Vec<Line> = self
            ...
            .map(|(i, orig)| {
                ...
                let on = *on;
                let selected = i == self.selection.selected().get() && self.focus == Focus::List;
    ...
    fn destination_lines(&self, theme: &Theme) -> Vec<Line<'static>> {
        ...
        for (i, orig) in self.visible_dest().into_iter().enumerate() {
            ...
            let selected =
                i == self.destination_selection.selected().get() && self.focus == Focus::List;
            let style = if selected {
                selection_style(true, sem)
            } else {
    ...
    fn render_buttons(
        ...
        let sem = &theme.semantic;
        let button = |label: &str, identity: Button| {
            let active = self.focus == Focus::Buttons && self.button == identity;
            if active {
                Span::styled(
                    format!(" {label} "),
                    Style::default()
