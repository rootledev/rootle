...
use rootle_provider::{
    Capabilities, GitRef, Provider, ProviderResult, RepoId, RepoInfo, SearchCodeResult, SearchItem,
    Sha, TreeResult,
};
...
fn build_declared(config: &Config, kind: &str) -> (Arc<dyn Provider>, BuildOutcome) {
    let r = match rootle_manager::ProviderReference::parse(kind) {
        Ok(r) => r,
        Err(e) => {
            return (
                github_fallback(config),
...
fn try_spawn_stdio(
    ...
    let cache_dir = rootle_provider::paths::cache_dir().map(|d| {
        d.join("rootle")
            .join("providers")
